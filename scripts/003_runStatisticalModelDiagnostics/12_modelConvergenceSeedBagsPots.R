...
mcmcSamples <- readRDS(mcmcSampleFiles[[grep("seedBagPotExperimentsPosteriorSamples-reduced3.RDS", mcmcSampleFiles)]])
...
outputDirectory = "outputs/003_runStatisticalModelDiagnostics/01_tracePlots/6_seedBagsPots/"

# - +Graphical checks on trace plots: survival ----
f(x = "mu0_s", model = "belowground-reduced")
f(x = "sigma0_s", model = "belowground-reduced")

# - +Graphical checks on trace plots: germination ----
f(x = "mu0_g", model = "belowground-reduced")
f(x = "sigma0_g", model = "belowground-reduced")

# - +Graphical checks on trace plots: s0 ----
f(x = "mu0_s0", model = "belowground-reduced")
f(x = "sigma0_s0", model = "belowground-reduced")
...
# - +Plot Rhat and HW ----
par(mfrow = c(1, 1))
jpeg(filename = paste0(outputDirectory, "rhat-hw-seedSurvival-pots", ".jpeg"), 
     quality = 75)
...
par(mfrow = c(1, 1))
jpeg(filename = paste0(outputDirectory, "rhat-hw-germination-pots", ".jpeg"), 
  quality = 75)
...
# - +Plot Rhat and HW ----
par(mfrow = c(1, 1))
jpeg(filename = paste0(outputDirectory, "rhat-hw-s0-pots", ".jpeg"), quality = 75)
# plot distribution of Rhat values
