...
mcmcSamples <- readRDS(mcmcSampleFiles[[grep("seedlingSurvivalPosteriorSamples.RDS", mcmcSampleFiles)]])
...
par(mfrow=c(1,1))
jpeg(filename = paste0(outputDirectory, "rhat-hw-seedlingSurvival", ".jpeg"), 
  quality = 75)
# plot distribution of Rhat values
hist(rhat, col = "black", border = "white", breaks = 25, 
     main=NULL,
     xlab ="R-hat values",cex.axis=1.2,cex.lab=1.4)
title(main = "A. Distribution of R-hat for seedling survival",adj=0,cex.main=1.4)
...
text(max(dt$mids), 0.75 * max(dt$counts), paste0("% passing (chain 1): ", 
  p[1]), pos = 2,cex=1.4)
text(max(dt$mids), 0.7 * max(dt$counts), paste0("% passing (chain 2): ", 
  p[2]), pos = 2,cex=1.4)
text(max(dt$mids), 0.65 * max(dt$counts), paste0("% passing (chain 3): ", 
  p[3]), pos = 2,cex=1.4)
dev.off()
