...
mu_g <- MCMCchains(mcmcSamplesViability, params = "mu_g")[,index.g3]
mu_g = cbind(mu0_g , mu_g)

# viability assays
mu0_v<-MCMCchains(mcmcSamplesViability, params = "mu0_v")
mu_v <- MCMCchains(mcmcSamplesViability, params = "mu_v")[,index.g3]
mu_v = cbind(mu0_v , mu_v)
...
# - ++January, age 1 seeds ----
nu_ratio2=nu[,index.g1[1:20]]*(nu[,index.g2[1:20]]/nu[,index.g1[1:20]])^(1/3)
# - ++January, age 2 seeds ----
...
s5.v = ((sigma1*(1-gamma1)*sigma2*sigma3*(1-gamma2)*sigma4*sigma5)*(gamma3+(1-gamma3)*nu_23.inter))/(s1.v*(1-g1.v)*s2.v*s3.v*(1-g2.v*s4.v))
...
saveRDS(bayesianEstimates,paste0("/Users/Gregor/Dropbox/dataLibrary/chapter-3/","seedRateEstimatesBayesianCombinedPopulationLevel.rds"))
