...
#mcmcSamples.p <- readRDS(mcmcSampleDirectory[[grep("seedlingSurvivalSamplesSimplePlots.RDS",mcmcSampleDirectory)]])
...
i = 1
index= data$site==i
n.max= max(data$seedlingNumber[index])
...
for(j in 1:15){
  ...
  points(rep(j,length(tmp)),tmp,col='gray',pch=1)
...
#mu0.p  <- MCMCchains(mcmcSamples.p,params="mu0")
#mu0.p <- apply(boot::inv.logit(mu0.p),2,quantile,c(.025,.5,.975))
#mu_year.p  <- MCMCchains(mcmcSamples.p,params="mu")
#mu.year.p.summary <- apply(boot::inv.logit(mu_year.p),2,quantile,c(.025,.5,.975))

#plot(mu0[2,],mu0.p[2,],ylim=c(0,1),xlim=c(0,1))

# plot(mu.year.summary[2,][order(mu.year.summary[2,])],
#      mu.year.p.summary[2,][order(mu.year.p.summary[2,])])
# abline(a=0,b=1)
# 
# mu_survival <- MCMCchains(mcmcSamples.p,params="mu.s")
# mu.summary <- apply(boot::inv.logit(mu_survival),2,quantile,c(.025,.5,.975))
...
siteYearTransectIndex = data$siteYearTransectIndex
d=data.frame(site=site,year=year,siteYearIndex=siteYearIndex,
             siteYearTransectIndex,siteYearTransectIndex,
             n=n,y=y,p=y/n)
d.summary = d %>% 
  dplyr::group_by(siteYearTransectIndex) %>% 
  dplyr::summarise(p=mean(p,na.rm=TRUE),n=sum(n))

ref<-d %>% dplyr::select(siteYearIndex,siteYearTransectIndex) %>% unique
...
df<-d.summary %>%
  dplyr::left_join(ref,by='siteYearTransectIndex') %>%
  dplyr::left_join(d.summary2,by=c('siteYearIndex'))

par(mfrow=c(1,1))
plot(mu.year.summary[2,],d.summary2$p.yr,xlim=c(0,1),ylim=c(0,1),
     pch=NA,xlab="Predicted seedling survival (mean of posterior)",
     ylab="Observed seedling survival (mean of observations)")
abline(a=0,b=1)
points(d.summary2$p.yr,mu.year.summary[2,],pch=1)

# plot(d.summary$p,mu.summary[2,],xlim=c(0,1),ylim=c(0,1),pch=NA,xlab="Mean (MLE)",ylab="Posterior median from model")
# abline(a=0,b=1)
# points(d.summary$p,mu.summary[2,],pch=1)

# add points from no pooling model
# points(d.summary$p,mu.summary.np[2,],pch=1,col='coral')
...
for(i in 1:20){
  ...
  plot(mu.year.summary[2,index],d.summary2$p.yr[index],xlim=c(0,1),ylim=c(0,1),
       pch=NA,xlab="Mean (MLE)",ylab="Posterior median",
       main=siteNames[i])
  abline(a=0,b=1)
  points(mu.year.summary[2,index],d.summary2$p.yr[index],pch=16)
  segments(y0=d.summary2$p.yr[index],x0=mu.year.summary[1,index],x1=mu.year.summary[3,index])
...
plot(d.summary2$n.yr,mu.year.summary[2,]-d.summary2$p.yr,ylim=c(-1,1),pch=1,
     xlab="Sample size of year*site combination",ylab="Posterior mean - mean of observations")
segments(x0=d.summary2$n.yr,y0=mu.year.summary[1,]-d.summary2$p.yr,y1=mu.year.summary[3,]-d.summary2$p.yr)
abline(h=0)
...
for(i in 1:20){
  ...
  for(j in 1:length(index)){
   index.j = unique(data$siteYearIndex[index[j]==data$siteYearIndex])
     data$fruitplNumber[data$siteYearIndex == index.j]
  i}
...
# plot(d.summary$n,mu.summary.np[2,]-d.summary$p,ylim=c(-1,1),pch=1,xlab="Mean (MLE)",ylab="Posterior median",
#      main=siteNames[i])
# segments(x0=d.summary$n,y0=mu.summary.np[1,]-d.summary$p,y1=mu.summary.np[3,]-d.summary$p)
# abline(h=0)

# plot(mu.summary[2,]-d.summary$p,mu.summary.np[2,]-d.summary$p,
#      xlim=c(-1,1),ylim=c(-1,1),
#      pch=1,xlab="Pooled estimate - MLE",ylab="Unpooled estimate - MLE")
# abline(a=0,b=1)
# abline(v=0,h=0)
...
#   index = unique(index)
#   
#   plot(d.summary$n[index],mu.summary[2,index]-d.summary$p[index],ylim=c(-1,1),pch=NA,xlab="Sample size",ylab="Posterior median - mean (MLE)",
#        main=siteNames[i])
#   abline(a=0,b=1)
#   points(d.summary$n[index],mu.summary[2,index]-d.summary$p[index],pch=16)
#   segments(x0=d.summary$n[index],y0=mu.summary[1,index]-d.summary$p[index],y1=mu.summary[3,index]-d.summary$p[index])
#   
#   #  points(d.summary$p[index],mu.summary.np[2,index],pch=16,col='coral')
#   # segments(x0=d.summary$p[index],y0=mu.summary.np[1,index],y1=mu.summary.np[3,index],col='coral')
# }
...
pdf(file=paste0(outputDirectory,"seedlingSurvivalFruitingSimple-pvals.pdf"),height=6,width=6)
