...
data <- readRDS(modelDataDirectory[[grep("seedsPerFruitData.RDS",modelDataDirectory)]])

# - +Read in MCMC samples ----
mcmcSampleDirectory <- paste0(mcmcDirectory,list.files(mcmcDirectory))
mcmcSamples <- readRDS(mcmcSampleDirectory[[grep("seedsPerFruitSamplesSimple.RDS",mcmcSampleDirectory)]])
...
pdf(file=paste0(outputDirectory,"seedsPerUndamagedFruitSimple-ppc-population.pdf"),height=6,width=6)
...
pdf(file=paste0(outputDirectory,"seedsPerUndamagedFruitSimple-pvals.pdf"),height=6,width=6)
...
for(j in 1:length(years)){
  ...
  f(x=start[1]+offset[j],y=sdno.min.mat[,j],width=.2)
  
  f(x=start[2]+offset[j],y=sdno.max.mat[,j],width=.2)
  
  f(x=start[3]+offset[j],y=sdno.mean.mat[,j],width=.2)
  
  f(x=start[4]+offset[j],y=sdno.sd.mat[,j],width=.2)
