...
modelVariables <- c("yearViab","n_yearViab","ageViab","bag","n_bag",
...
initsFun = function(){
  temp = list(initsMu0(cols=2),initsMu0(cols=2), initsSigma0(cols=2), initsSigma0(cols=2), initsSigma(), initsSigma(),
...
out <- clusterEvalQ(cl, {
  ...
  jm = jags.model(file=paste0(modelDirectory,"jags-viability-age3.R"),
...
saveRDS(samples.rjags,file=paste0(outMCMCDirectory,"viabilityTrialSamples.RDS"))
