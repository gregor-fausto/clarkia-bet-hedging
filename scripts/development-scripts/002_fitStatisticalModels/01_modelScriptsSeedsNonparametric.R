...
initsSigmaWeak2 <- function(rows = data$n_siteSurvival, cols = 4){
  matrix(extraDistr::rhnorm(n = rows*cols, sigma = 1), rows, cols)
}
...
initsSigmaLessWeak <- function(rows = data$n_sitePlot, cols = data$n_yearPlot){
  matrix(extraDistr::rhnorm(n = rows*cols, sigma = 2), rows, cols)
}
...
initsFun = function(){
  ...
  temp = list(initsMu0Mat(rows=data$n_siteSurvival, cols=4), initsSigma0LessWeakMat(rows=data$n_siteSurvival,cols=4),
              initsSigmaWeak(rows=data$n_siteSurvival,cols=12),
              initsMu0Mat(cols=2), initsSigma0LessWeakMat(cols=2), initsSigmaWeak2(cols=6),
              initsMu0(), initsSigma0LessWeak(), initsSigmaWeak3(),
              initsSeedlings[2,],  initsSeedlings[1,],
              "base::Mersenne-Twister", runif(1, 1, 2000))

  names(temp) = c("mu0_s","sigma0_s","sigma_s",
...
# model parameters
parsToMonitor = c("mu0_s","sigma0_s","mu_s","sigma_s",
                  "mu0_g", "sigma0_g","mu_g", "sigma_g",
                  "mu0_s0", "sigma0_s0", "mu_s0", "sigma_s0",
                  "s0_1","s0_2")
# model checking
...
out <- clusterEvalQ(cl, {
  ...
  jm = jags.model(file=paste0(modelDirectory,"jags-seeds-nonparametric-twoyear.R"),
...
saveRDS(samples.rjags,file=paste0(outMCMCDirectory,"seedBagExperimentSamplesNonparametric.RDS"))
