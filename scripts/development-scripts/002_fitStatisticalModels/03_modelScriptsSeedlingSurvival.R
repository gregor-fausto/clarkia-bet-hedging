...
modelVariables <- c("transect","n_transect","position","n_position")
...
initsSigma <- function(samps = data$n_siteYearIndex){
  extraDistr::rhnorm(n = samps, sigma = 1)
}
...
initsFun = function(){
  temp = list(initsMu0(), initsSigma0(), initsSigma(), 
...
out <- clusterEvalQ(cl, {
  library(rjags)
  jm = jags.model(file=paste0(modelDirectory,"jags-seedlingSurvivalSimple.R"),
...
saveRDS(samples.rjags,file=paste0(outMCMCDirectory,"seedlingSurvivalSamplesSimple.RDS"))
