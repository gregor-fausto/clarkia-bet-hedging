...
mcmcSamples <- readRDS(mcmcSampleFiles[[grep("fruitsPerPlantSamplesSimple.RDS", mcmcSampleFiles)]])
...
f(x = "nu_tfe", model = "fruits_tfe_OLRE")
f(x = "sigma0_tfe", model = "fruits_tfe_OLRE")
f(x = "sigma.tfe.overdisp", model = "fruits_tfe_OLRE")

# print trace plots for each parameter
f(x = "nu_tot", model = "fruits_tot")
f(x = "sigma0_tot", model = "fruits_tot")
f(x = "sigma_tot", model = "fruits_tot")

# print trace plots for each parameter
f(x = "mu0", model = "propDamage")
f(x = "sigma0", model = "propDamage")
f(x = "sigma", model = "propDamage")

# - +Recover chains for TFE ----
all.chains = MCMCchains(mcmcSamples, params = c("nu_tfe", "sigma0_tfe"), mcmc.list = TRUE)
...
all.chains = MCMCchains(mcmcSamples, params = c("nu_tot", "sigma0_tot"), mcmc.list = TRUE)
...
# - +Plot Rhat and HW ----
par(mfrow = c(1, 1))
jpeg(filename = paste0(outputDirectory, "rhat-heidelberg-fruits-prop-damaged", ".jpeg"), 
     quality = 75)
# plot distribution of Rhat values
