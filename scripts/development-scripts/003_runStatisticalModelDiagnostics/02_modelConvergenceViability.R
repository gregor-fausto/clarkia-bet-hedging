...
f(x = "sigma_g", model = "germinationTrials")
f(x = "mu0_v", model = "viabilityTrials")
f(x = "sigma0_v", model = "viabilityTrials")
f(x = "sigma_v", model = "viabilityTrials")

# - +Recover chains ----
all.chains = MCMCchains(mcmcSamples, params = c("mu0_g", "sigma0_g", "sigma_g", "mu0_v", 
  "sigma0_v", "sigma_v"), mcmc.list = TRUE)
...
par(mfrow = c(1, 1))
jpeg(filename = paste0(outputDirectory, "rhat-hw-viability", ".jpeg"), quality = 75)
# plot distribution of Rhat values
hist(rhat, col = "black", border = "white", breaks = 25, main = "Distribution of R-hat; viability")
