...
# this was not done in JAGS for computational reasons
# script would get stuck - using too much memory?
# so we're doing it here

# get the mean of the poisson distribution for each observation
z_tfe=MCMCchains(mcmcSamples, params = "z_tfe")

# simulate 5000 datasets
n.sim = 5000
# draw 5000 values from the posterior for z_tfe
draws.iter=sample(1:dim(z_tfe)[1],n.sim)
z_tfe.sub = z_tfe[draws.iter,]
n.obs = dim(z_tfe.sub)[2]
# for each observation, simulate values from a Poisson
# taking z_tfe as lambda in the Poisson distribution
y_tfe.sim = matrix(NA,nrow=n.sim,ncol=dim(z_tfe.sub)[2])
for(i in 1:n.sim){
  y_tfe.sim[i,]<-rpois(n.obs,z_tfe.sub[i,])
}
# these are the simulated values
y.sim = y_tfe.sim
...
# - ++Calculate chi-squared for observed and simulated values ----
...
# use 6 sites - 4 with poor fit and 4 with good fit
...
for(i in (1:length(years))[ years %in% plot.yr]){
  if(colSums(p.chi.mat>.95,na.rm=TRUE)[i]==0){
    ...
    for(j in c(site.index,add.index)){
      ...
      # plot as a discrete density if there is 1 observation
...
# script would get stuck - using too much memory?
# so we're doing it here
# repeat the steps taken for total fruit equivalents
