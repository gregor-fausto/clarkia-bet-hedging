...
mu0.p  <- MCMCchains(mcmcSamples,params="mu0")
mu0.p <- apply(boot::inv.logit(mu0.p),2,quantile,c(.025,.5,.975))

mu_year.p  <- MCMCchains(mcmcSamples,params="mu")
...
n.iter=dim(y.sim)[1]
subsample = sample(n.iter,1000)
y.sim.sub = y.sim[subsample,]
years=2006:2020
...
for(j in 1:length(years)){
  ...
  f.box(x=start[1]+offset[j],y=seedlings.min.mat[,j],width=.2)
  
  f.box(x=start[2]+offset[j],y=seedlings.max.mat[,j],width=.2)
  
  f.box(x=start[3]+offset[j],y=seedlings.mean.mat[,j],width=.2)
  
  f.box(x=start[4]+offset[j],y=seedlings.sd.mat[,j],width=.2)
